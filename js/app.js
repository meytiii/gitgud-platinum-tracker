...
// ========================================================
// 6. 21st.dev CANVAS2D ASCII-ART & PIXEL SPRITE ENGINE
// ========================================================
let asciiAnimationId = null;
let asciiRenderMode = 'pixel'; // 'pixel' | 'characters' | 'matrix' | 'dither'

function initAsciiSprite() {
    const canvas = document.getElementById('ascii-sprite-canvas');
    if (!canvas) return;

    const ctx = canvas.getContext('2d', { willReadFrequently: true });
    if (!ctx) return;

    const modeLabel = document.getElementById('ascii-mode-label');
    const toggleBtns = document.querySelectorAll('.ascii-toggle-btn');
    toggleBtns.forEach(btn => {
        btn.addEventListener('click', (e) => {
            e.stopPropagation();
            toggleBtns.forEach(b => b.classList.remove('active'));
            btn.classList.add('active');
            asciiRenderMode = btn.getAttribute('data-mode') || 'pixel';
            if (modeLabel) {
                const labels = {
                    pixel: 'PIXEL FX',
                    characters: 'ASCII RUNES',
                    matrix: 'MATRIX RAIN',
                    dither: 'DITHER SHADER'
                };
                modeLabel.textContent = labels[asciiRenderMode] || 'PIXEL FX';
            }
        });
    });
    ...
    const cellSize = 14;
    const cols = Math.floor(targetSize / cellSize);
    const rows = Math.floor(targetSize / cellSize);
    const matrixDrops = [];
    for (let c = 0; c < cols; c++) {
        matrixDrops[c] = Math.floor(Math.random() * rows);
    }
    ...
    function renderFrame(time) {
        if (!canvas.isConnected) {
            asciiAnimationId = requestAnimationFrame(renderFrame);
            return;
        }

        const elapsed = (time - startTime) * 0.001;
        const pulse = 1 + Math.sin(elapsed * 3.0) * 0.15; // Pulse animation

        ctx.clearRect(0, 0, targetSize, targetSize);

        // 1. Draw source photo into offscreen buffer
        offCtx.clearRect(0, 0, targetSize, targetSize);
        offCtx.fillStyle = '#080706';
        offCtx.fillRect(0, 0, targetSize, targetSize);

        if (img.complete && img.naturalWidth > 0) {
            offCtx.globalAlpha = 0.4;
            offCtx.drawImage(img, 15, 15, targetSize - 30, targetSize - 30);
            offCtx.globalAlpha = 1.0;
            const scale = 0.85 * (1 + Math.sin(elapsed * 1.5) * 0.04);
            const w = (targetSize - 50) * scale;
            const h = (targetSize - 50) * scale;
            const ox = (targetSize - w) / 2;
            const oy = (targetSize - h) / 2;
            offCtx.drawImage(img, ox, oy, w, h);
        } else {
            offCtx.fillStyle = '#d4a359';
            offCtx.beginPath();
            offCtx.arc(targetSize / 2, targetSize / 2, 60 * pulse, 0, Math.PI * 2);
            offCtx.fill();
        }

        // 2. Sample luminance and rasterize
        let imgData;
        try {
            imgData = offCtx.getImageData(0, 0, targetSize, targetSize);
        } catch (e) {
            return;
        }
        ...
        for (let y = 0; y < targetSize; y += cellSize) {
            for (let x = 0; x < targetSize; x += cellSize) {
                const cx = Math.min(x + Math.floor(cellSize / 2), targetSize - 1);
                const cy = Math.min(y + Math.floor(cellSize / 2), targetSize - 1);
                const idx = (cy * targetSize + cx) * 4;
                ...
                // Color adjustments: brightness (12), contrast (115)
                r = Math.min(255, Math.max(0, ((r / 255 - 0.5) * 1.15 + 0.5) * 255 + 12));
                g = Math.min(255, Math.max(0, ((g / 255 - 0.5) * 1.15 + 0.5) * 255 + 12));
                b = Math.min(255, Math.max(0, ((b / 255 - 0.5) * 1.15 + 0.5) * 255 + 12));
                ...
                if (asciiRenderMode === 'characters') {
                    const charIdx = Math.floor(lum * (charSet.length - 1));
                    const glyph = charSet[charIdx];
                    ctx.font = `bold ${Math.floor(cellSize * 0.95 * pulse)}px 'JetBrains Mono', monospace`;
                    ctx.fillStyle = `rgba(${Math.round(r * 1.2)}, ${Math.round(g * 1.1)}, ${Math.round(b * 0.9)}, ${Math.min(1, lum + 0.25)})`;
                    ctx.textAlign = 'center';
                    ctx.textBaseline = 'middle';
                    ctx.fillText(glyph, x + cellSize / 2, y + cellSize / 2);
                } else if (asciiRenderMode === 'matrix') {
                    const isHead = (matrixDrops[colIndex] === rowIndex);
                    const isTrail = (matrixDrops[colIndex] >= rowIndex && matrixDrops[colIndex] - rowIndex < 5);
                    const glyphs = '0123456789ABCDEF';
                    const randGlyph = glyphs[Math.floor((colIndex * 7 + rowIndex * 13 + Math.floor(elapsed * 10)) % glyphs.length)];
                    ctx.font = `bold ${Math.floor(cellSize * 0.85)}px 'JetBrains Mono', monospace`;
                    ctx.textAlign = 'center';
                    ctx.textBaseline = 'middle';
                    if (isHead) ctx.fillStyle = '#ffffff';
                    else if (isTrail || lum > 0.3) ctx.fillStyle = `rgba(212, 163, 89, ${isTrail ? 0.95 : lum * 0.85})`;
                    else ctx.fillStyle = `rgba(180, 130, 60, 0.2)`;
                    ctx.fillText(randGlyph, x + cellSize / 2, y + cellSize / 2);
                } else if (asciiRenderMode === 'dither') {
                    const bayer4x4 = [
                        [ 0,  8,  2, 10],
                        [12,  4, 14,  6],
                        [ 3, 11,  1,  9],
                    ...
                    const threshold = bayer4x4[rowIndex % 4][colIndex % 4] / 16;
                    if (lum > threshold * 0.75) {
                        ctx.fillStyle = `rgba(212, 163, 89, ${lum})`;
                        ctx.fillRect(x + 1, y + 1, cellSize - 2, cellSize - 2);
                    }
                } else {
                    const pSize = Math.max(2, (cellSize - 3) * lum * pulse);
                    const offset = (cellSize - pSize) / 2;
                    ctx.fillStyle = `rgb(${Math.round(r)}, ${Math.round(g)}, ${Math.round(b)})`;
                    ctx.beginPath();
                    ctx.roundRect(x + offset, y + offset, pSize, pSize, 2);
                    ctx.fill();
                }
            }
        }
        ...
        if (Math.floor(time / 80) % 2 === 0) {
            for (let c = 0; c < cols; c++) {
                if (Math.random() > 0.65) matrixDrops[c] = (matrixDrops[c] + 1) % (rows + 4);
            }
        }

        // Post-effects: Vignette + Bloom
        const radGrad = ctx.createRadialGradient(targetSize / 2, targetSize / 2, 40, targetSize / 2, targetSize / 2, targetSize * 0.72);
        radGrad.addColorStop(0, 'rgba(0, 0, 0, 0)');
        radGrad.addColorStop(1, 'rgba(0, 0, 0, 0.76)');
    ...
    if (asciiAnimationId) cancelAnimationFrame(asciiAnimationId);
    asciiAnimationId = requestAnimationFrame(renderFrame);
}

// ========================================================
// 7. PLATINUM CHECKLIST TRACKER ENGINE
// ========================================================
